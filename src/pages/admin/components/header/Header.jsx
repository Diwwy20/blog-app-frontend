...
import { RiMenuLine, RiCloseLargeLine, RiDashboard2Fill } from "react-icons/ri";
...
const Header = () => {
  ...
  return (
    <header className="flex h-fit w-full items-center justify-between p-4 lg:h-full lg:max-w-[300px] lg:flex-col lg:items-start lg:justify-start lg:p-0">
      {/* logo */}
      <Link to="/">
        <img src={images.Logo} alt="logo" className="w-[140px] lg:hidden" />
      </Link>
      {/* menu burger icon */}
      <div className="cursor-pointer lg:hidden">
        {isMenuActive ? (
          <RiCloseLargeLine className="w-6 h-6" onClick={toggleMenuHandler} />
        ) : (
          <RiMenuLine className="w-6 h-6" onClick={toggleMenuHandler} />
        )}
      </div>
      {/* sidebar container */}
      {isMenuActive && (
