...
const Users = () => {
  ...
  const handleAdminCheck = (event, userId) => {
    ...
    if(window.confirm("Do you want to change the admin status of the user?")){
      mutateUpdateUser({
        isAdmin: event.target.checked, userId
      })
    } else {
      ...
      {usersData?.data.map((user) => (
        <tr key={user._id}>
          ...
          <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
            <input
              type="checkbox"
              className={`${user.admin ? "border-none" : ""} d-checkbox disabled:bg-orange-400 disabled:opacity-100 checked:bg-[url('../public/images/check.png')] bg-cover checked:disabled:bg-none rounded-full`}
